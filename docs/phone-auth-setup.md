...
Vercel Production 환경에 아래 항목을 설정했고 운영 사이트 전환을 완료했습니다. 운영 도메인은 `https://www.amorlab.kr`입니다. Preview 환경은 별도 개발 DB 설정이 필요합니다.
...
주문 저장과 웹 인증은 Neon·Better Auth를 사용합니다. 모바일 이용권 발급은 Supabase에 연결됩니다. 기존 이미지 URL은 Supabase Storage를 계속 가리킬 수 있습니다. 원본 주문과 Edge Functions는 삭제·변경하지 않았습니다. 전환 전 구 사이트에서 추가 주문이 발생했다면 최종 이관을 다시 실행해야 하며, 이관 스크립트는 기존 Neon 주문을 덮어쓰지 않습니다. 기존 주문의 상태가 원본에서 변경됐다면 별도 대조가 필요합니다. 구 Edge Functions를 종료할 때는 공유 프로젝트의 다른 테넌트 영향을 먼저 확인하세요.
...
- 서버 환경변수: `MOBILE_SUPABASE_URL`, `MOBILE_SUPABASE_SERVICE_ROLE_KEY`.
  ...
  키는 클라이언트에 노출하지 않습니다. 로컬과 Vercel Production에 설정했습니다.
...
`/admin`의 **새 주문 관리자 알림**에서 사용 여부·수신번호·문자 문구를 관리합니다.
초기값은 사용, 수신번호 `01097224668`이며 주문자·프로그램·금액과 입금 확인 요청을 보냅니다.
설정 저장은 문자 발송을 하지 않으며 이후 접수되는 주문부터 적용됩니다.

주문과 `admin_order_messages`를 한 트랜잭션에 저장하고, 커밋 후 Next.js `after`에서
SOLAPI LMS를 발송합니다. 주문번호는 고유하므로 같은 주문 재요청으로 중복 알림을 만들지 않습니다.
당시 수신번호와 문구를 저장하므로 설정 변경이 기존 알림의 재발송 대상을 바꾸지 않습니다.
문자 실패는 접수된 주문을 취소하지 않습니다. 마스터에서 대기/명시적 실패만 재발송할 수 있고,
결과 불명·처리 중은 SOLAPI 확인 후 별도 대응합니다. 기존 주문은 소급 발송하지 않습니다.
검증: `pnpm test:order-alert` (실제 문자 발송 없는 격리 DB + 가짜 전송기).
Neon migration `0005`를 개발/운영에 적용합니다. 기존 SOLAPI 환경변수를 재사용합니다.

## 입금 확인 취소와 환불 완료

마스터의 입금 확인 주문에서 `입금 확인 취소` 또는 `환불 완료 처리`를 선택하고 사유를 입력합니다. 환불은 **관리자가 주문 금액 전액을 직접 송금한 후** 기록합니다. 자동 송금이나 환불 안내 문자 발송은 하지 않습니다.

- 취소는 앱 회수 완료 후 `pending`, 환불은 `refunded`로 바뀝니다. 미완료 상태는 `reversing`이며 `회수 재시도`로 복구합니다.
- 가입 전 이용권도 회수되며 이후 같은 번호로 가입해도 활성화되지 않습니다. 가입한 고객은 해당 주문의 **미사용 기간만** 회수합니다. 이미 사용한 기간, 다른 구매, 기존 무기한 이용권은 유지합니다.
- 연장 구매는 기록된 시작·종료 시각으로 남은 기간을 계산하고 뒤에 연결된 구매 기간도 함께 이동합니다. 앱에서 종료일이 별도로 단축되어 발급 기록과 충돌하면 자동 처리를 중단합니다. 이 경우 앱 이용권 변경 내역을 먼저 조사해야 합니다.
- 새 발급 이력이 없는 이관 주문은 앱에서 직접 이용권을 확인·회수하거나 회수 대상이 없음을 확인해야 합니다. 화면의 확인 항목과 처리 사유가 기록됩니다.
- 취소 후 다시 입금 확인하면 새로운 `issuance_id`로 발급합니다. 이전 회수 요청이나 늦게 도착한 발급 요청이 새 이용권에 영향을 주지 않습니다. 기존 발급 건과 이전 버전의 동시 배포는 `issuance_id ?? order_id`로 호환합니다.
- `order_reversals`에 회수 요청, 담당자, 사유, 수동 확인 및 처리 상태를 보존하고 `order_events`에 요청·완료를 기록합니다. 입금 문자가 발송 중이면 취소를 차단합니다.

추가 환경변수는 없습니다. Neon의 `0006`과 Supabase의 `amor_landing_reversals` 마이그레이션을 먼저 적용한 뒤 배포합니다. 검증은 `pnpm test:reversals`, `pnpm test:mobile-grants`로 실행하며 실제 문자나 송금은 발생하지 않습니다.
